...
import { axiosArticles, axiosArticlesbyDiscount } from "../apiCall";
...
export default function News() {
    ...
    const [order, setOrder] = useState('discount desc');
    ...
    useEffect(() => {
        const getArticles = async () => {
            try {
                const response = await axiosArticlesbyDiscount(currentPage, nameParam, order, true);
                setArticles(response);

                const popularResponse = await axiosArticles(1, nameParam, 'id desc');
                setPopularArticles(popularResponse);

                const responseNextPage = await axiosArticlesbyDiscount(currentPage + 1, nameParam, order, true);
                    ...
                    <>
                        ...
                        {currentPage === 1 && nameParam === '' && (
                            <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
                                <h1 className="text-4xl font-bold mb-20">Nouveautés</h1>
                                <div
                        ...
                        <div className="min-h-screen mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
                            {currentPage === 1 ? (
                                <div className={`flex items-center ${nameParam === "" ? "justify-between" : "justify-end"} mb-20`}>
                                    {currentPage === 1 && nameParam === '' ? (
