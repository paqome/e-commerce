...
export default function Article() {
    ...
    useEffect(() => {
        const getArticles = async () => {
            try {
                const response = await axiosArticlesbyCategory(id_category, currentPage, nameParam, order);
                setArticles(response);

                const responseNextPage = await axiosArticlesbyCategory(id_category, currentPage + 1, nameParam, order);
